...
const FullPurchaseOrder = ({ purchaseOrder }: FullPurchaseOrderProps) => {
    const { companyName, companyAddress, currencySymbol, vendors, themeSettings, platformSettings } = useAppContext();
    ...
    const docFooter = (
        <footer className={cn('text-center text-muted-foreground text-sm mt-auto', template === 'letterhead' ? 'pt-8' : 'pt-16')} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
    ...
    return (
        <DialogContent className="sm:max-w-4xl max-h-[90vh] p-0 gap-0 printable-area-container flex flex-col">
            ...
            {template === 'lined' ? (
              ...
              <div className="printable-area a4-doc force-light-doc bg-white text-black p-4 sm:p-6 overflow-y-auto flex-1 min-h-0 flex flex-col">
              ...
              <div className={cn('printable-area a4-doc force-light-doc bg-white text-black overflow-y-auto flex-1 min-h-0 flex flex-col', template === 'letterhead' ? 'relative p-4 sm:p-6' : 'p-4 sm:p-8')}>
