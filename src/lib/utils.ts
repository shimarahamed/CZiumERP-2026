...
/**
 * Copies text to the clipboard. navigator.clipboard is only available in
 * secure contexts (HTTPS or localhost) — on a plain-HTTP LAN host it's
 * undefined, and calling it throws "Copy to clipboard is not supported in
 * this browser". Falls back to the legacy execCommand approach there.
 * Returns whether the copy succeeded, so callers can toast appropriately.
 */
